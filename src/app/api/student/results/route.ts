...
import { Student, TestAttempt } from '@/models';

/**
 * Student-only route to retrieve their test results.
...
export async function GET(request: Request) {
  ...
  const results = await TestAttempt.find({ student: targetStudentId })
    .populate<{ test: { title: string; totalMarks: number; subject?: { name: string } } }>({
      path: 'test',
